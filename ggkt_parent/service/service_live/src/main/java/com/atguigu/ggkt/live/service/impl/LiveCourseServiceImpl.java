...
import com.atguigu.ggkt.utils.utils.DateUtil;
...
public class LiveCourseServiceImpl extends ServiceImpl<LiveCourseMapper, LiveCourse> implements LiveCourseService {
    ...
    @Resource
    private LiveCourseConfigService liveCourseConfigService; //直播课程配置服务
    @Resource
    private LiveCourseGoodsService liveCourseGoodsService;   //直播课程商品服务
    @Resource
    ...
    public IPage<LiveCourse> selectPage(Page<LiveCourse> pageParam) {
        ...
        //遍历获取直播课程list集合
        liveCourseList.stream().forEach(item -> {
            this.selectPagination(item);
        });
...
//    直播课程添加
        ...
        try {
           String res =  mtCloud.courseAdd(liveCourse.getCourseName(),
            ...
            if(Integer.parseInt(commonResult.getCode()) == MTCloud.CODE_SUCCESS) { //成功
                ...
                throw new GgktException(20001 , "直播课程添加失败"+getmsg);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public LiveCourseFormVo getLiveCourseVo(Long id) {
        ...
        LiveCourse liveCourse = baseMapper.selectById(id);
        //获取直播课程描述信息
        LiveCourseDescription liveCourseDescription =  liveCourseDescriptionService.getCourseDescription(id);
        //封装
...
//                *  options 		可选参数
        ...
        try {
            ...
            if(Integer.parseInt(commonResult.getCode()) == MTCloud.CODE_SUCCESS) {
                ...
                LiveCourseDescription liveCourseDescription =
                        liveCourseDescriptionService.getCourseDescription(liveCourse.getId());
    ...
    public LiveCourseConfigVo getCourseConfig(Long id) {
        //根据课程id查询配置信息
        LiveCourseConfig liveCourseConfig = liveCourseConfigService.getCourseConfigId(id);
        //封装LivecourseConfigVo
        LiveCourseConfigVo liveCourseConfigVo = new LiveCourseConfigVo();
        if(liveCourseConfigVo != null) {
            //查询直播课程商品列表
            List<LiveCourseGoods> goodsList = liveCourseGoodsService.getLiveGoodsListId(id);
            //封装到ivecourseConfigVo里面
            BeanUtils.copyProperties(liveCourseConfig,liveCourseConfigVo);
            //疯转商品列表
            liveCourseConfigVo.setLiveCourseGoodsList(goodsList);
        }
...
//    获取用户access_token
    ...
    public JSONObject getAccessToken(Long id, Long userId) {
        ...
        try {
            ...
            CommonResult<JSONObject> result = JSON.parseObject(res, CommonResult.class);
    ...
    private LiveCourse selectPagination(LiveCourse liveCourse) {
        //每个课程讲师id
        Long courseId = liveCourse.getCourseId();
        //根据id查询讲师信息
        Teacher teacherLive = teacherFeignClient.getTeacherLive(courseId);
        //进行封装
        if(teacherLive != null) {
